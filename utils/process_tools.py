...
def clean_ball_tracking_data(csv_path, output_dir="processed"):
    """
    Cleans ball tracking data:
    - Interpolates `posX` and `posY`
    - Fills `team_possesion` using majority vote within a 3-frame window
    - Saves the cleaned file in `processed/ball_tracking_cleaned.csv`
    """
    os.makedirs(output_dir, exist_ok=True)  # Ensure output directory exists
    
    df = pd.read_csv(csv_path)
    df[['posX', 'posY']] = df[['posX', 'posY']].interpolate(method='linear')

    def fill_team_possession(series, window=3):
        filled_series = series.copy()
        for idx in series[series.isna()].index:
            nearby_values = series.iloc[max(0, idx - window) : min(len(series), idx + window + 1)].dropna()
            filled_series.at[idx] = nearby_values.mode().iloc[0] if not nearby_values.empty else series.ffill().bfill().at[idx]
        return filled_series

    df['team_possession'] = fill_team_possession(df['team_possession'])
    output_file = os.path.join(output_dir, "ball_tracking_cleaned.csv")
    df.to_csv(output_file, index=False)

    print(f"✅ Cleaning complete. File saved at: {output_file}")
...
def extract_video_clips(video_path, times, output_dir="clips"):
...
def get_times_by_pos(data, position):
    return [int(event["time_in_minutes"]) for event in data if event["pos"] == position]
        ...
        if ball.xyxy.any() and players.xyxy.any():
            ...
            if team_in_possession:
                ...
                st.session_state.possession_events.append({
                    ...
                    "team_possession":team_in_possession,
                    "pos": zone,
