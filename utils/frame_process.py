...
model = load_player_detection_model(direct_link=direct_link)
...
def process_frame_with_convex_hull(frame, team_classifier, team_id=None):
    ...
    # Fonction pour tracer un convex hull rempli
    def draw_filled_convex_hull(annotated_frame, points, color, alpha=0.3):
        if points:
            ...
            for i in range(len(hull_points)):
                cv2.line(annotated_frame, hull_points[i], hull_points[(i+1) % len(hull_points)], (255, 255, 0), 2)
    ...
    for i in range(len(players_detections)):
        x1, y1, x2, y2 = map(int, players_detections.xyxy[i])
        if players_detections.class_id[i] == team_id or team_id is None:
            cv2.rectangle(annotated_frame, (x1, y1), (x2, y2), team_color, 2)
            label = f"Team {players_detections.class_id[i]}"
            cv2.putText(annotated_frame, label, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, team_color, 2)
...
def draw_radar_view(frame, CONFIG, team_classifier,passes_mode, type='tactical'):
    """
...
def calculate_realistic_optimal_passes(frame, CONFIG, team_classifier, max_passes=3):
    # Traitement des détections
    results = model.predict(frame, conf=0.3)
    detections = sv.Detections(
        xyxy=results[0].boxes.xyxy.detach().cpu().numpy(),
...
def annotate_frame_with_hulls(frame,team_id,team_classifier,CONFIG):
    """
