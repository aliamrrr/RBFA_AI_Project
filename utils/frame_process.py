...
def process_frame_with_convex_hull(frame, team_classifier, input_type="Image", video_path=None, team_id=None):
    """
    Fonction qui effectue l'inférence sur une frame ou une vidéo complète, détecte les objets,
    puis les annote avec leurs couleurs respectives, tout en traçant des convex hulls
    autour des joueurs des équipes demandées.

    Args:
    - frame (ndarray): La frame de la vidéo à traiter (si input_type est 'Image').
    - team_classifier (object): Le modèle de classification des équipes pour les joueurs.
    - input_type (str): "Image" ou "Video" selon le type d'entrée.
    - video_path (str, optional): Chemin vers la vidéo à traiter si input_type est "Video".
    - team_id (list, optional): Liste des identifiants des équipes à tracer (0 ou 1). Si None, trace les deux équipes.

    Retourne:
    - frame_annotated (ndarray) si input_type est "Image".
    - Génère une vidéo annotée si input_type est "Video".
    """
    ...
    def draw_filled_convex_hull(annotated_frame, points, color, alpha=0.15):
        if points:
            ...
            line_color = (255, 255, 0) if color == (0, 255, 0) else (255, 0, 0)
    ...
    if input_type == "Image":
        ...
        for i in range(len(players_detections)):
            ...
            team_players[players_detections.class_id[i]].append(((x1 + x2) / 2, (y1 + y2) / 2))
        if team_id is None or team_id == 0:
            draw_filled_convex_hull(annotated_frame, team_players[0], (0, 0, 255))
        if team_id is None or team_id == 1:
            draw_filled_convex_hull(annotated_frame, team_players[1], (0, 255, 0))
        return annotated_frame
    
    elif input_type == "Video" and video_path:
        frame_count = 0 
        ...
        while cap.isOpened():
            ...
            if not ret:
                break
            annotated_frame = process_frame_with_convex_hull(frame, team_classifier, input_type="Image", team_id=team_id)
            out.write(annotated_frame)
        cap.release()
        out.release()
        st.success("Vidéo traitée et enregistrée sous 'output_video.mp4'")


def generated_lines_video(video_path, team_classifier, config,
    ...
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    out = cv2.VideoWriter("output_video_lines.mp4", fourcc, fps_output, (width, height))
    ...
    while cap.isOpened():
        ...
        if players.xyxy.any():
            ...
            team_1_indices = np.where(players.class_id == 1)[0]  
            ...
            if len(team_1_indices) > 2:
                ...
                for cluster_id in range(3):
                    ...
                    if len(role_indices) >= 3:  # On ne dessine un convex hull que si 3 joueurs ou +
                        print("Convex Hull tracé")
                        ...
                        hull_points = cluster_points[hull.vertices]  # Récupération des points du contour convexe
                        
                        # Remplir le convex hull sur l'overlay
                        cv2.fillConvexPoly(overlay, np.array(hull_points, dtype=np.int32), polygon_colors[0])
    ...
    cap.release()
    out.release()
    st.success(f"✅ Vidéo annotée sauvegardée sous output_video.mp4")
